...
GitHub Copilot shell — keep it short and point back to the core to avoid drift.

- **Response shape**: conclusion first, then evidence (files, line ranges, or
  command output), then the next action.
- **Stay local and scoped**: no default network, no hidden telemetry, no copying
  private material into suggestions, no forced hooks; require an explicit path
  for anything outside the project.
...
中文要点：先结论后证据再下一步；默认不联网/不加遥测/不把私有材料写进建议/不强装 hook/只在项目内读写；先验证再断言；动密钥·部署·依赖·全局设置前先停。完整规则见 `_core-contract.md`。

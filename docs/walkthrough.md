...
first missing structure. It runs offline and uploads nothing. For a fully worked
example, see [docs/sample-room.md](./sample-room.md).
...
Evidence, Pro acceleration, and Method lines:
...
读它返回的那一张[断点卡](./breakpoint-card.md)。它点名第一个缺失的结构。它离线运行、不上传
任何东西。完整跑通的例子见 [docs/sample-room.md](./sample-room.md)。
...
CLI 还会打印 Evidence、Pro acceleration、方法说明几行：
